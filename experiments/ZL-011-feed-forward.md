...
This is one fixed-seed imitation experiment in a deterministic toy world—not evidence about human intelligence, independent discovery, reinforcement learning, or statistical confidence. The [protocol](ZL-011-protocol.md) remains unchanged at commit `875c75ec5bc8c0b39179202ff570b6a5d4e10fbd`, on base `b22cf4990f10a694998e4a571b534ece8d46cd43`. The **original evaluation evidence** is bound to the dirty worktree and explicit source hashes, not a retroactively assigned clean release commit. That evaluator did not commit or publish. The subsequent owner-authorized delivery uses reviewed source and the **same frozen model bytes**; CI regenerates source-bound replay artifacts without retraining. [Preserved original receipts](../evidence/neural/README.md) distinguish the first scientific measurements from release re-verification.
...
**Scientific work completed:** actual frozen-model evaluation, original and expanded held-out domains at the stated scope, both all-state parity paths, counts/hash verification, and this negative-result report. **Not claimed:** expanded held-out planner comparisons or an artifact-budget pass over all external evidence combined.

**Delivery:** the owner subsequently authorized independent review, commit/push, PR CI/artifact checks, merge and live verification. The final release state is recorded by the repository PR/deployment checks and canonical project delivery receipt, not inferred from this pre-publication report. The [replay](../neural.html), [release outcome JSON](../neural.json), [held-out JSON](../neural-heldout.json), and [release verification JSON](../evaluation.json) are generated from the reviewed source; the [original measured evaluation](../evidence/neural/evaluation/evaluation.json) and all original model hashes above remain unchanged. CI uses frozen models and dependency-free JavaScript, explicitly skips unavailable optional browser/NumPy checks, and performs real-artifact Node replay checks even without Playwright. Local/downloaded/live browser checks are separate gates, not implied by a green Node test.
